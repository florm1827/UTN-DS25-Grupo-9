...
/**
 * Crear reserva
 * - USER: nombre = nombre (no puede setearlo), estado PENDIENTE
 * - ADMIN: puede setear nombre/tipo, estado ACEPTADA automáticamente
 * - Valida solapamiento contra ACEPTADAS del día/cancha
 */
...
/**
 * -  listar aceptadas para la grilla del día
 * - Devuelve reservas ACEPTADAS del día consultado
 * - Incluye TURNO_FIJO activos  cuya fecha base <= día consultado
 * - filtro opcional por cancha
 */
export const listarReservasAceptadas = async (req, res) => {
  try {
    ...
    // fijos activos (fecha base <= fecha consultada) y no dados de baja
    ...
    // clonar fijos con fecha del día consultado
    const fijosParaElDia = fijos.map(r => ({ ...r, fecha: new Date(start) }))
...
/**
 * Cancelar mi solicitud pendiente (USER)
 */
...
/**
 * Listar pendientes (ADMIN) con filtros (fecha/cancha/nombre)
 */
...
/**
 * Aceptar (ADMIN) con validación de solapamiento
 */
...
/**
 * Listar aceptadas (ADMIN) con filtros (fecha/cancha/nombre)
 */
...
/**
 * Actualizar (ADMIN) con validación de solapamiento
 */
export const actualizarReservaAdmin = async (req, res) => {
  try {
    ...
    const TIPOS = TIPOS_ADMIN
    let tipoFinal = reserva.tipo
    if (tipo) {
      if (!TIPOS.includes(tipo)) return res.status(400).json({ ok: false, msg: 'Tipo de reserva no permitido' })
      tipoFinal = tipo
    }
    ...
    if (existeSolape) {
      return res.status(400).json({ ok: false, msg: 'No se puede modificar: ya hay una reserva aceptada en ese horario.' })
    }
    ...
    const actualizada = await prisma.reserva.update({
      ...
      data: {
        ...
        horaInicio: nuevaHoraInicio,
        horaFin: nuevaHoraFin,
        nombre: (typeof nombre === 'string' && nombre.trim() !== '') ? nombre.trim() : reserva.nombre,
...
/**
 * Dar de baja (ADMIN)
 * - Si es TURNO_FIJO → marcar baja=true para que no aparezca en fechas futuras
 */
